package com.atomscat.config.interceptor;
...
import com.atomscat.config.properties.AtomscatIpLimitProperties;
import com.atomscat.config.properties.AtomscatLimitProperties;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Autowired
    private AtomscatLimitProperties limitProperties;

    @Autowired
    private AtomscatIpLimitProperties ipLimitProperties;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if(ipLimitProperties.getEnable()) {
            ...
            if (StrUtil.isBlank(token1)) {
                throw new AtomscatException("你手速怎么这么快，请点慢一点");
            }
        }
        ...
        if(limitProperties.getEnable()){
            ...
            if (StrUtil.isBlank(token2)) {
                throw new AtomscatException("当前访问总人数太多啦，请稍后再试");
            }
        }
        ...
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (StrUtil.isBlank(token3)) {
                    throw new AtomscatException("当前访问人数太多啦，请稍后再试");
                }
            }
        }catch (Exception e){
